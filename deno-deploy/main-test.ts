/**
 * Test harness copy of main.ts — imported by *_test.ts only.
 * Production Deno Deploy continues to run main.ts (Deno.serve, no test exports).
 * Keep behaviour in sync when changing API routes: copy the change into this file
 * or re-copy main.ts and re-apply handleRequest + helper exports at the bottom.
 * -------------------------------------------------------
...
function corsHeaders(_req?: Request): Record<string, string> {
  ...
  return {
    "access-control-allow-headers": "authorization, content-type, x-auth-token, accept, origin, range, content-disposition, cache-control, pragma, x-requested-with",
    "access-control-allow-methods": "GET,POST,PATCH,PUT,DELETE,OPTIONS",
...
/** Exported for Deno tests (`api_test.ts`). Production uses Deno.serve below. */
