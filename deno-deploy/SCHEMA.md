...
| Immutable Geo reference data is cached for 1 day (`public, max-age=86400`) | Districts, ACs, Mandals, and Revenue Divisions are static reference tables (`districts`, `assembly_constituencies`, `mandals`, `revenue_divisions`). No in-app write path exists, so 24-hour public caching is safe without complex cache-invalidation logic. Eliminates redundant network round-trips for mobile field surveyors on poor 3G/4G networks. | **Settled rule.** `/api/geo`, `/api/geo/children`, `/api/geo/mandals`, `/api/geo/revenue_divisions` return `Cache-Control: public, max-age=86400`. Do not add TTL invalidation or strip the cache header. |
