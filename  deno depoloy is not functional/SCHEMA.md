...
`voice_required BOOLEAN` (field app shows the mic step only when true;
Client Admin with `can_record_voice`, or Super Admin, may toggle).
`voice_time_limit INT` minutes auto-stop, `0` = no limit. **Writable only
by Super Admin** on POST `/api/surveys` and PUT `/api/surveys/:id`.
